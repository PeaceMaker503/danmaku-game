...
{
    public partial class bEnemyForm : Form
    {
            ...
            if (grid.Rows[args.RowIndex].Cells[args.ColumnIndex].Value != DataGridViewHelper.EMPTY)
            {
                if (grid.Rows[args.RowIndex].Cells[args.ColumnIndex].Value is string)
                {
                    ...
                    if (newFormattedValue != DataGridViewHelper.BAD_FORMAT)
                    {
                        ...
                        if (grid.Columns[args.ColumnIndex].Name == DataGridViewHelper.MoveGridColumnsName.DESTINATION)
                        {
                            grid.Rows[args.RowIndex].Cells[DataGridViewHelper.MoveGridColumnsName.DIRECTION].Value = DataGridViewHelper.EMPTY;
                        }
                        else if (grid.Columns[args.ColumnIndex].Name == DataGridViewHelper.MoveGridColumnsName.DIRECTION)
                        {
                            grid.Rows[args.RowIndex].Cells[DataGridViewHelper.MoveGridColumnsName.DESTINATION].Value = DataGridViewHelper.EMPTY;
                        }
                    }
                    else
        ...
        {
            this.dataGridView1.Rows.Add(JsonFloat.Zero, DataGridViewHelper.LONG_DEFAULT, JsonVector2.Zero, DataGridViewHelper.EMPTY, JsonFloat.Zero, DataGridViewHelper.EMPTY, DataGridViewHelper.EMPTY);
        }
    }
}
